...
def async_ingest_document_task(
    ...
    db, rag_service = get_services()
    doc_record = db.query(EnterpriseDocument).filter(EnterpriseDocument.id == document_id).first()
...
def async_execute_ingestion_job_task(job_id: str):
    ...
    try:
        ...
        if job.source_type == "s3":
            ...
            from app.connectors.db_connector import RelationalDBConnector
        ...
        for raw_doc in connector.fetch_documents():
            rag_service.ingest_enterprise_document(
